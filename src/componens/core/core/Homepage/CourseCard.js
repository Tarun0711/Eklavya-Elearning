...
const CourseCard = ({cardData, currentCard, setCurrentCard}) => {
  if (!cardData) return null; // Add this line to return null if cardData is undefined
  return (
    <div className = {`w-[360px] lg:w-[30%] text-richblack-25 h-[300px] box-border cursor-pointer
        ...
        <div className = {`flex justify-between ${currentCard === cardData.heading ? "text-blue-300" : "text-richblack-300" } px-6 py-3 font-medium`} >  
          <p className="flex items-center gap-2 text-[16px]">  <HiUsers /> {cardData?.courses?.level} </p>           {/* Level :- Beginner*/}
          <p className="flex items-center gap-2 text-[16px]"> <ImTree /> {cardData?.courses?.lessionNumber} Lession </p>       {/* Flow Chart :- 6 Lession*/}
        </div>
